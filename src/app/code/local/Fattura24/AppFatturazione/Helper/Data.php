...
class Fattura24_AppFatturazione_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function apiCall($fattura24_api_url, $apiKey, $send_data = array())
    {
    ...
    public function traceObject($description, $object)
    {
    ...
    public function make_strings($v1, $v2)
    {
    ...
    public function writeElement($xml, $tag, $value)
    {
        if (strlen($value) !== 0) {
    ...
    public function createXml($order, $documentType)
    {
        ...
        if ($shippingAddress != null) {
            $customerAddress = $this->make_strings(
                $billingAddress->getStreet(),
                $shippingAddress->getStreet()
            );
            $customerPostcode = $this->make_strings(
                $billingAddress->getPostcode(),
                $shippingAddress->getPostcode()
            );
            $customerCity = $this->make_strings(
                $billingAddress->getCity(),
                $shippingAddress->getCity()
            );
            $customerProvince = $this->make_strings(
                $billingAddress->getRegion(),
                $shippingAddress->getRegion()
            );
            $customerCountry = $this->make_strings(
                $billingAddress->getCountryId(),
                $shippingAddress->getCountryId()
            );
            $customerCellPhone = $this->make_strings(
                $billingAddress->getTelephone(),
                $shippingAddress->getTelephone()
            );
            $vatId = $this->make_strings(
        ...
        } elseif ($paymentMethodCode == 'checkmo') {
    ...
    */
    ...
    public function getDocIdInvoice($order)
    {
        try {
            $resource = Mage::getSingleton('core/resource');
    ...
    public function getDocIdOrder($order)
    {
        try {
            $resource = Mage::getSingleton('core/resource');
            $readConnection = $resource->getConnection('core_read');
